...
open class WBDevice: NSObject, Jsonifiable, CBPeripheralDelegate {
    ...
    func didConnect() {
        NSLog("\(self) didConnect")
        self.connectTransactions.forEach { $0.resolveAsSuccess() }

        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            NSLog("\(self) starting GATT services discovery")
    ...
    func didDisconnect(error: Error?) {
        ...
        // ✅ Success disconnect (user initiated)
        self.disconnectTM.apply{$0.resolveAsSuccess()}
    }
    ...
    func triage(_ tview: DeviceTransactionView) {
        ...
        case .connectGATT:
            guard let man = self.manager else {
                transaction.resolveAsFailure(withMessage: "Manager missing")
                return
            }
            
            NSLog("⚡ connectGATT - BLITZ FAST (1s)...")
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
                NSLog("⚡ BLITZ GATT CONNECT")
                man.centralManager.connect(self.peripheral)
            }
            
            transaction.addCompletionHandler({ transaction, _ in
